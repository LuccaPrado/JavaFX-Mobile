...
- [mipastgt/JFXToolsAndDemos — "JFX-Android" article](https://github.com/mipastgt/JFXToolsAndDemos/blob/master/docs/articles/JFX-Android/JFX-Android.adoc) — a real internally-shipped JavaFX-on-Android app (map navigation, GPS tracking over MQTT), used here as independent confirmation that arbitrary third-party Maven libraries (SLF4J, Jackson, Commons IO/lang3/math3, Eclipse Paho MQTT, ikonli, miglayout) cross-compile fine through `gluonfx-maven-plugin` — no Gluon-specific fork needed — and that desktop-first development (build/test on the plain JVM, cross-compile for Android only once the UI/logic already works) is the workflow other real projects converge on too.
